...
const ChiTietSanPham = () => {
  ...
  const handleGuiDanhGia = async (e) => {
    ...
    try {
      ...
      if (res.data) {
        ...
        addToast({
          message:
            "Đánh giá đã được gửi! Vui lòng chờ Admin duyệt để hiển thị.",
          type: "success",
        });
      }
    } catch (error) {
  ...
  const addToCart = (buyNow = false) => {
    ...
    const existingItemIndex = currentCart.findIndex(
      (item) =>
        item.id === sanPham.id &&
        item.rental_days === (buyNow ? 0 : soNgayThue),
    );
    ...
    const itemToAdd = {
      ...
      quantity: 1,
      price_to_use: buyNow
        ? sanPham.purchase_price
        : sanPham.rental_price_day * soNgayThue,
    };
  ...
  const gallery =
    Array.isArray(sanPham.gallery_images) && sanPham.gallery_images.length > 0
        ...
        <div className="product-detail-container">
          <div className="product-detail-gallery">
            ...
            >
              <img
                ...
                onError={(e) =>
                  (e.target.src =
                    "https://placehold.co/600x800?text=IVIE+Studio")
                }
                style={{
                  maxHeight: "520px",
                  objectFit: "contain",
                      ...
                      style={{
                        ...
                        border:
                          galleryIndex === idx
                            ...
                            : "1.5px solid #eee",
                        boxShadow:
                          galleryIndex === idx ? "0 2px 8px #b5941033" : "none",
                        background: "#fff",
                      }}
                    />
                    <div
          ...
          <div className="product-detail-info">
            <span className="product-detail-category">
              {sanPham.category === "wedding_modern"
                ? "Váy Cưới Hiện Đại"
                : sanPham.category === "traditional"
                  ? "Áo Dài Truyền Thống"
                  : "Phụ Kiện"}
            </span>
            <h1 className="product-detail-name">{sanPham.name}</h1>
            <span className="product-detail-code">
              Sản phẩm thủ công độc quyền • Mã #{sanPham.code}
            </span>

            <div className="product-detail-price-box">
              <div className="price-item">
                <span className="price-label">Giá thuê (Ưu đãi ngày):</span>
                <span className="price-value">
                  {dinhDangGia(sanPham.rental_price_day)}
                </span>
              </div>
              <div className="price-item">
                <span className="price-label">Sở hữu vĩnh viễn:</span>
                <span className="price-value highlight">
                  {dinhDangGia(sanPham.purchase_price)}
                </span>
              </div>
            </div>
            ...
            <div className="product-attributes">
              <div className="attribute-card">
                <span className="attribute-label">Chất liệu cao cấp</span>
                <span className="attribute-value">
                  {sanPham.fabric_type || "Ren Pháp & Lụa tơ tằm"}
                </span>
              </div>
              <div className="attribute-card">
                <span className="attribute-label">Sắc màu chủ đạo</span>
                <span className="attribute-value">
                  {sanPham.color || "Trắng Ivory / Ngọc trai"}
                </span>
              </div>
              <div className="attribute-card">
                <span className="attribute-label">Form dáng gợi ý</span>
                <span className="attribute-value">
                  {sanPham.recommended_size || "Cao: 1m55-1m75 | Nặng: 45-65kg"}
                </span>
              </div>
              <div className="attribute-card">
                <span className="attribute-label">Phong cách Makeup</span>
                <span className="attribute-value">
                  {sanPham.makeup_tone || "Tone Tây sang trọng / Hàn Quốc"}
                </span>
              </div>
            </div>
            ...
            >
              ...
              <p>
                {sanPham.description ||
                  "Mẫu thiết kế tinh xảo đến từng đường kim mũi chỉ, sử dụng những chất liệu tốt nhất để tôn vinh vẻ đẹp kiêu sa của bạn trong ngày trọng đại nhất cuộc đời."}
              </p>
            </div>
            ...
            <div className="rental-options">
              ...
              <div className="days-selector">
                {[1, 2, 3, 4, 5, 6, 7].map((day) => (
                  <button
                    key={day}
                    className={`day-btn ${soNgayThue === day ? "active" : ""}`}
                    onClick={() => setSoNgayThue(day)}
                  >
                    {day}
                  </button>
                ))}
        ...
        {goiYNam.length > 0 && (
          <section className="recommendations-section">
            <div className="section-header">
              <h2>Kết Hợp Hoàn Hảo</h2>
              <p>Đề xuất các mẫu Vest nam phù hợp để sánh đôi cùng nàng</p>
            </div>
        ...
        <section className="reviews-section">
          <div className="reviews-container">
            <div className="section-header">
              ...
              <p>Chia sẻ trải nghiệm tuyệt vời của bạn cùng IVIE STUDIO</p>
            </div>
            ...
            <form className="review-form" onSubmit={handleGuiDanhGia}>
              ...
              <div className="rating-input">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    key={star}
                    type="button"
                    className={`star-btn ${soSao >= star ? "active" : ""}`}
              ...
              <div className="form-group">
                ...
                <textarea
                  ...
                  placeholder="Chia sẻ cảm nhận của bạn về chất liệu, form dáng..."
                  required
                ></textarea>
              </div>
                ...
                danhGia.map((dg) => (
                  <div key={dg.id} className="review-card">
                    ...
                    {dg.image_url && (
                      ...
                      >
                        <img
                          src={layUrlHinhAnh(dg.image_url)}
                          alt="Review"
                          style={{
                            maxWidth: "200px",
                            borderRadius: "12px",
